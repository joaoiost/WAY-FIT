...
const TYPE_COLORS = {
  ...
  'Avaliação Física': '#0EA5E9', 'Consulta Nutricional': '#22C55E',
};
const GROUP_TO_TYPE = {
...
function ApptCard({ appt, onClick, isSelected }) {
  return (
    <div
      ...
      style={{
        background: isSelected ? appt.color : `${appt.color}1c`,
    ...
    >
      <p style={{ margin: 0, fontSize: 11, fontWeight: 700, color: isSelected ? 'white' : 'var(--gray-700)' }}>{(appt.time || '').slice(0, 5)}</p>
      <p style={{ margin: '1px 0 0', fontSize: 11, color: isSelected ? 'rgba(255,255,255,0.85)' : 'var(--gray-500)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
  ...
  return (
    <div
      ...
      style={{
        ...
        border: isSelected ? `1.5px solid ${appt.color}` : '1.5px solid var(--border-light)',
      ...
      <div style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 8 }}>
        ...
        >
          <div style={{ width: 28, height: 28, borderRadius: '50%', background: 'var(--bg-page)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="var(--gray-400)" strokeWidth="2"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14H6L5 6"/><path d="M10 11v6M14 11v6"/></svg>
          </div>
...
export default function WeeklyAgenda() {
  ...
  const openNewModal = (date) => {
    setEditingId(null);
    setSelectedAppt(null);
    setForm({ studentId: '', date: formatDate(date), time: '08:00', type: 'Musculação', notes: '', repeat: false, repeatWeeks: '4', daysOfWeek: [] });
    setModal(true);
  };

  return (
    <div id="agenda-section" className="card card-0" style={{ marginBottom: 24, padding: '18px 20px 20px' }}>
      {/* Header */}
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, marginBottom: 16, flexWrap: 'wrap' }}>
        <div>
          <h3 className="section-title">Agenda da semana</h3>
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          ...
          <button className="btn-primary agenda-new-btn" onClick={() => openNewModal(new Date())}>
      ...
      <div className="agenda-desktop-view">
        <div className="agenda-outer-grid" style={{ display: 'grid', gridTemplateColumns: selectedAppt ? '1fr 300px' : '1fr', gap: 16, alignItems: 'start' }}>
          <div style={{ background: 'var(--bg-surface)', borderRadius: 12, border: '1px solid var(--border-light)', overflow: 'hidden' }}>
            <div style={{ overflowX: 'auto' }}>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', borderBottom: '2px solid var(--border-light)', minWidth: 560 }}>
                  ...
                  return (
                    <button
                      ...
                      style={{
                        ...
                        background: isToday ? '#EFF6FF' : 'var(--bg-page)',
                        transition: 'background 0.15s',
                      }}
                      onMouseEnter={e => { if (!isToday) e.currentTarget.style.background = 'var(--border-light)'; }}
                      onMouseLeave={e => { if (!isToday) e.currentTarget.style.background = 'var(--bg-page)'; }}
                      title={`Agendar aula em ${date.toLocaleDateString('pt-BR')}`}
                    >
                      <p style={{ margin: 0, fontSize: 10, fontWeight: 700, color: isToday ? '#3B82F6' : 'var(--gray-400)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>
                        {DAYS_PT[date.getDay()]}
                      </p>
                      <p style={{ margin: '4px 0 0', fontSize: 22, fontWeight: 800, color: isToday ? '#3B82F6' : 'var(--gray-900)' }}>
                        {date.getDate()}
                      </p>
                      <Plus size={12} color={isToday ? '#3B82F6' : 'var(--gray-400)'} style={{ marginTop: 2 }} />
                    </button>
                  );
                })}
              </div>
                ...
                {weekDates.map((date, i) => {
                  ...
                  return (
                    <div key={i} style={{
                      ...
                      background: isToday ? 'rgba(59,130,246,0.06)' : 'var(--bg-surface)',
            ...
            return (
              <button
                ...
                style={{
                  ...
                  background: isSelected ? '#3B82F6' : isToday ? '#EFF6FF' : 'var(--bg-surface)',
              ...
              >
                <span style={{ fontSize: 10, fontWeight: 700, color: isSelected ? 'rgba(255,255,255,0.75)' : 'var(--gray-400)', textTransform: 'uppercase', letterSpacing: '0.04em' }}>
                  {DAYS_PT[date.getDay()]}
                </span>
                <span style={{ fontSize: 19, fontWeight: 800, color: isSelected ? 'white' : isToday ? '#3B82F6' : 'var(--gray-900)', marginTop: 2, lineHeight: 1 }}>
                  {date.getDate()}
                </span>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {mobileDayAppts.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '52px 0 40px', color: 'var(--gray-400)' }}>
              <Calendar size={36} color="var(--border)" style={{ display: 'block', margin: '0 auto 12px' }} />
      ...
      <button
        ...
        onClick={() => openNewModal(mobileDay)}
              ...
              {[
                ...
                <button
                  ...
                  style={{
                    ...
                    background: scheduleMode === m.key ? 'var(--bg-surface)' : 'transparent',
                    color: scheduleMode === m.key ? 'var(--gray-900)' : 'var(--gray-500)',
                    ...
                    {[{v:1,l:'Seg'},{v:2,l:'Ter'},{v:3,l:'Qua'},{v:4,l:'Qui'},{v:5,l:'Sex'},{v:6,l:'Sáb'},{v:0,l:'Dom'}].map(d => {
                      ...
                      return (
                        <button
                          ...
                          style={{
                            flex: 1, padding: '8px 4px', borderRadius: 8, border: `2px solid ${on ? '#3B82F6' : 'var(--border)'}`,
                            background: on ? '#EFF6FF' : 'var(--bg-page)', color: on ? '#1D4ED8' : 'var(--gray-500)',
