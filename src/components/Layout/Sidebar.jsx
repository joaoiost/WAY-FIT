...
export default function Sidebar() {
  ...
  const NavGroup = ({ items }) => items.map(({ to, icon: Icon, label, end }) => (
    ...
    <>
      ...
      <aside className={`app-sidebar${open ? ' sidebar-open' : ''}`}
        ...
        <nav style={{ display: 'flex', flexDirection: 'column', gap: 2, flex: 1 }}>
          ...
          <p style={{ fontSize: 11, fontWeight: 700, color: '#C4C9D4', textTransform: 'uppercase', letterSpacing: '0.08em', padding: '0 8px', marginBottom: 4 }}>
            Ferramentas
          </p>
        ...
        <Link to="/dashboard/perfil" onClick={close}
          style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px', borderRadius: 10, border: '1px solid #F1F5F9', background: '#F9FAFB', textDecoration: 'none', marginBottom: 8, marginTop: 12 }}>
