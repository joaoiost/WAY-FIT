  ...
  return (
    <div style={{ flex: 1 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
        ...
        <span style={{ fontSize: 12, fontWeight: 800, color: 'var(--gray-900)' }}>{value}<span style={{ color: '#9CA3AF', fontWeight: 500 }}>/{goal || '—'}</span></span>
      </div>
...
function FoodRow({ item, onChange, onDelete }) {
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '2fr 80px 100px 60px 60px 60px 60px 36px', gap: 6, alignItems: 'center', padding: '8px 0', borderBottom: '1px solid #F9FAFB' }}>
      <input value={item.name} onChange={e => set('name', e.target.value)} placeholder="Alimento" style={{ padding: '7px 10px', borderRadius: 8, border: '1.5px solid var(--border)', fontSize: 13, color: 'var(--gray-900)', outline: 'none' }} />
      <input value={item.quantity} onChange={e => set('quantity', e.target.value)} placeholder="Qtd" type="number" style={{ padding: '7px 8px', borderRadius: 8, border: '1.5px solid var(--border)', fontSize: 13, color: 'var(--gray-900)', outline: 'none', textAlign: 'center' }} />
      <select value={item.unit} onChange={e => set('unit', e.target.value)} style={{ padding: '7px 6px', borderRadius: 8, border: '1.5px solid var(--border)', fontSize: 12, color: 'var(--gray-900)', background: 'var(--bg-surface)', outline: 'none' }}>
        {UNITS.map(u => <option key={u} value={u}>{u}</option>)}
      </select>
      {['kcal', 'protein', 'carbs', 'fat'].map(k => (
        <input key={k} value={item[k]} onChange={e => set(k, e.target.value)} placeholder="0" type="number" style={{ padding: '7px 6px', borderRadius: 8, border: '1.5px solid var(--border)', fontSize: 13, color: 'var(--gray-900)', outline: 'none', textAlign: 'center' }} />
      ))}
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px 16px', borderBottom: open ? '1px solid #F3F4F6' : 'none' }}>
        ...
        <div onClick={() => setOpen(o => !o)} style={{ flex: 1, minWidth: 0, cursor: 'pointer' }}>
          <input value={meal.name} onChange={e => { e.stopPropagation(); onChange({ ...meal, name: e.target.value }); }} onClick={e => e.stopPropagation()}
            style={{ width: '100%', border: 'none', background: 'transparent', fontSize: 14, fontWeight: 700, color: 'var(--gray-900)', outline: 'none', cursor: 'text', pointerEvents: 'auto' }} />
    ...
    <div className="page-padding" style={{ flex: 1, maxWidth: 900, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 20 }}>
        <button onClick={() => navigate(`/dashboard/alunos/${studentId}`)}
          style={{ width: 40, height: 40, borderRadius: '50%', background: 'var(--bg-surface)', border: '1px solid var(--border)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
          <ArrowLeft size={18} color="#374151" />
        </button>
        <div style={{ flex: 1 }}>
          <input value={name} onChange={e => setName(e.target.value)}
            style={{ fontSize: 22, fontWeight: 900, color: 'var(--gray-900)', border: 'none', background: 'transparent', outline: 'none', width: '100%' }} />
          <p style={{ margin: '2px 0 0', fontSize: 13, color: '#9CA3AF' }}>{student?.name || 'Aluno'}</p>
        </div>
        <button onClick={save} disabled={saving}
          style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '10px 20px', background: saved ? '#10B981' : 'var(--accent)', border: 'none', borderRadius: 12, cursor: 'pointer', color: 'white', fontSize: 14, fontWeight: 700 }}>
      ...
      <div style={{ background: 'var(--bg-surface)', borderRadius: 16, padding: '16px 20px', marginBottom: 20, border: '1px solid var(--border)', boxShadow: '0 1px 4px rgba(0,0,0,0.05)' }}>
      ...
      <button onClick={addMeal} style={{ width: '100%', padding: '12px', background: 'var(--bg-surface)', border: '2px dashed var(--border)', borderRadius: 14, color: '#3B82F6', fontSize: 14, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, marginBottom: 20 }}>
        <Plus size={16} /> Adicionar refeição
      </button>

      {/* Observações */}
      <div style={{ background: 'var(--bg-surface)', borderRadius: 14, padding: '16px', border: '1px solid var(--border)', marginBottom: 32 }}>
        ...
        <textarea value={notes} onChange={e => setNotes(e.target.value)} rows={3} placeholder="Substituições permitidas, dicas, restrições..."
          style={{ width: '100%', padding: '10px 12px', borderRadius: 10, border: '1.5px solid var(--border)', fontSize: 14, color: 'var(--gray-700)', resize: 'vertical', outline: 'none', fontFamily: 'inherit', boxSizing: 'border-box' }} />
      </div>
