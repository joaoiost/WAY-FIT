...
const CSS = `
  ...
  @keyframes aurora2 {
    0%,100%{transform:translate(0,0) scale(1)}
    40%{transform:translate(-9%,5%) scale(1.1)}
    70%{transform:translate(7%,-8%) scale(0.92)}
  }
  @keyframes aurora3 {
    0%,100%{transform:translate(0,0) scale(1)}
  ...
  @keyframes glowGreen {
    ...
    50%{box-shadow:0 0 44px rgba(16,185,129,0.75),0 0 70px rgba(16,185,129,0.3)}
  }
  @keyframes glowBlue {
    0%,100%{box-shadow:0 0 22px rgba(59,130,246,0.45)}
    50%{box-shadow:0 0 44px rgba(59,130,246,0.75),0 0 70px rgba(59,130,246,0.3)}
  }
  @keyframes sweep {
  ...
  .btn-submit:active:not(:disabled) { transform:translateY(0) scale(0.97); }
...
export default function StudentLogin() {
  ...
  /* ── Role Toggle (shared) ─────────────────────────────────── */
  const RoleToggle = ({ bg = 'rgba(255,255,255,0.08)', border = '1px solid rgba(255,255,255,0.12)', textOff = 'rgba(255,255,255,0.45)', textOn = 'white' }) => (
    <div style={{
      position: 'relative', display: 'flex',
      background: bg, borderRadius: 100, padding: 4,
      border, width: '100%',
    }}>
      <div style={{
        ...
        transition: 'left 0.4s cubic-bezier(0.34,1.56,0.64,1), background 0.35s ease, box-shadow 0.35s ease',
        pointerEvents: 'none',
      }} />
      {[{ r: 'student', label: 'Aluno', Icon: User }, { r: 'personal', label: 'Personal', Icon: Dumbbell }].map(({ r, label, Icon }) => (
        <button key={r} className="toggle-btn" onClick={() => switchRole(r)}
          style={{
  ...
  /* ── Form fields (shared) ─────────────────────────────────── */
  const FormFields = () => (
    <form key={formKey} onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      {error && (
      ...
      <div style={{ animation: 'formSlideIn 0.4s ease 0.05s both' }}>
        <label style={{ display: 'block', fontSize: 13, fontWeight: 600, color: '#374151', marginBottom: 6 }}>Email</label>
        <input type="email" value={email} onChange={e => setEmail(e.target.value)}
          placeholder="seu@email.com" required autoFocus
          style={{ width: '100%', padding: '13px 14px', borderRadius: 12, border: '1.5px solid #E5E7EB', fontSize: 15, boxSizing: 'border-box', background: '#F9FAFB', color: '#111827', outline: 'none', transition: 'border-color 0.2s, box-shadow 0.2s, background 0.2s' }}
          onFocus={e => { e.target.style.borderColor = accent; e.target.style.background = 'white'; e.target.style.boxShadow = `0 0 0 3px ${accent}1A`; }}
          onBlur={e => { e.target.style.borderColor = '#E5E7EB'; e.target.style.background = '#F9FAFB'; e.target.style.boxShadow = 'none'; }}
        />
      </div>
      <div style={{ animation: 'formSlideIn 0.4s ease 0.1s both' }}>
        ...
        <div style={{ position: 'relative' }}>
          <input type={showPass ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)}
            ...
            style={{ width: '100%', padding: '13px 44px 13px 14px', borderRadius: 12, border: '1.5px solid #E5E7EB', fontSize: 15, boxSizing: 'border-box', background: '#F9FAFB', color: '#111827', outline: 'none', transition: 'border-color 0.2s, box-shadow 0.2s, background 0.2s' }}
            onFocus={e => { e.target.style.borderColor = accent; e.target.style.background = 'white'; e.target.style.boxShadow = `0 0 0 3px ${accent}1A`; }}
            onBlur={e => { e.target.style.borderColor = '#E5E7EB'; e.target.style.background = '#F9FAFB'; e.target.style.boxShadow = 'none'; }}
          />
      ...
      <button type="submit" disabled={loading} className="btn-submit"
        style={{
          ...
          border: 'none', background: accentGrad,
          ...
          animation: 'formSlideIn 0.4s ease 0.15s both',
        }}>
        {loading
  ...
  return (
    <>
      ...
      <div className="login-mobile" style={{ minHeight: '100dvh', flexDirection: 'column', position: 'relative', overflow: 'hidden', background: '#080C17' }}>
        ...
        {/* Aurora blobs — color transitions with role */}
        ...
        {/* Floating glass icons — touch parallax */}
        ...
        <div style={{ padding: 'max(52px,env(safe-area-inset-top)) 24px 0', position: 'relative', zIndex: 2, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
          ...
          {/* Big headline */}
          <h1 style={{ margin: '0 0 6px', fontSize: 34, fontWeight: 900, color: 'white', letterSpacing: '-1.3px', textAlign: 'center', lineHeight: 1.1, animation: 'fadeInUp 0.55s ease 0.08s both' }}>
            Treine com{' '}
          ...
          <div style={{ display: 'flex', gap: 14, width: '100%', position: 'relative', animation: 'fadeInUp 0.6s ease 0.28s both' }}>
            ...
            <div style={{
              ...
              left: isStudent ? '25%' : '75%',
              transform: 'translateX(-50%)',
              width: 110, height: 55,
              borderRadius: '50%',
              background: isStudent ? 'rgba(16,185,129,0.6)' : 'rgba(59,130,246,0.6)',
              filter: 'blur(24px)',
              transition: 'left 0.45s cubic-bezier(0.34,1.56,0.64,1), background 0.4s ease',
              pointerEvents: 'none', zIndex: 0,
            }} />

            {[
              { r: 'student',  label: 'Aluno',    sub: 'Seus treinos',  Icon: User,     grad: 'linear-gradient(135deg,#10B981,#059669)', color: '#10B981', neonAnim: 'neonGreen 2.2s ease-in-out infinite' },
              { r: 'personal', label: 'Personal', sub: 'Painel pro',    Icon: Dumbbell, grad: 'linear-gradient(135deg,#3B82F6,#6366F1)', color: '#3B82F6', neonAnim: 'neonBlue  2.2s ease-in-out infinite' },
            ].map(({ r, label, sub, Icon, grad, color, neonAnim }) => {
              const active = role === r;
              const SPARKS = [
                { sx: '-32px', sy: '-26px' }, { sx: '0px',   sy: '-38px' },
                { sx: '32px',  sy: '-26px' }, { sx: '38px',  sy: '4px'  },
                { sx: '24px',  sy: '30px'  }, { sx: '-24px', sy: '30px' },
                { sx: '-38px', sy: '4px'   },
              ];
              return (
                <button key={r} onClick={() => switchRole(r)}
                  style={{
                    ...
                    animation: active ? neonAnim : 'none',
                  ...
                  {/* Liquid fill */}
                  {active && (
                    <div style={{ position: 'absolute', inset: 0, background: grad, animation: 'liquidFill 0.55s cubic-bezier(0.22,1,0.36,1) both', zIndex: 0 }} />
                  )}
                  ...
                  {/* Spark particles */}
                  {active && SPARKS.map((p, i) => (
                    <div key={`${formKey}-${i}`} style={{
                      position: 'absolute', top: '40%', left: '50%',
                      width: 5, height: 5, borderRadius: '50%',
                      background: 'rgba(255,255,255,0.9)',
                      '--sx': p.sx, '--sy': p.sy,
                      animation: `spark 0.7s ease-out ${i * 0.045}s both`,
                  ...
                  <div style={{ position: 'relative', zIndex: 3, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10 }}>
                    <div key={`${r}-${active ? 'on' : 'off'}`} style={{
                      width: 58, height: 58, borderRadius: 18,
                      background: active ? 'rgba(255,255,255,0.22)' : `${color}22`,
                      display: 'flex', alignItems: 'center', justifyContent: 'center',
                      animation: active ? 'iconSpin 0.55s cubic-bezier(0.34,1.56,0.64,1) both' : 'none',
                    ...
                    <div style={{ textAlign: 'center' }}>
                      <p style={{ margin: 0, fontSize: 19, fontWeight: 900, letterSpacing: '-0.4px', color: active ? 'white' : 'rgba(255,255,255,0.42)', transition: 'color 0.35s' }}>
                        {label}
                      </p>
                      <p style={{ margin: '3px 0 0', fontSize: 12, color: active ? 'rgba(255,255,255,0.72)' : 'rgba(255,255,255,0.24)', transition: 'color 0.35s' }}>
                        {sub}
                      </p>
                    </div>
        ...
        <div style={{ background: 'white', borderRadius: '30px 30px 0 0', padding: '10px 24px max(28px,env(safe-area-inset-bottom))', boxShadow: '0 -24px 80px rgba(0,0,0,0.45)', position: 'relative', zIndex: 3, animation: 'cardPop 0.6s cubic-bezier(0.22,1,0.36,1) 0.15s both' }}>
          ...
          {/* Header */}
          <div key={`mh-${role}`} style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20, animation: 'formSlideIn 0.35s ease both' }}>
            ...
            <div>
              ...
              <h2 style={{ margin: 0, fontSize: 20, fontWeight: 900, color: '#111827', letterSpacing: '-0.4px' }}>
                Bem-vindo de volta
              </h2>
            </div>
          </div>
          ...
          <p style={{ textAlign: 'center', marginTop: 18, fontSize: 13, color: '#9CA3AF' }}>
            Sem conta?{' '}
            <Link to="/registro" style={{ color: accent, fontWeight: 700, textDecoration: 'none', transition: 'color 0.35s' }}>
              Cadastre-se grátis
            </Link>
          </p>
        </div>
      ...
      <div className="login-desktop" style={{ minHeight: '100vh' }}>
        ...
        <div ref={heroRef} style={{ flex: '1 1 55%', position: 'relative', overflow: 'hidden', padding: '60px 56px', display: 'flex', flexDirection: 'column', justifyContent: 'center', background: '#FAFAFA', minHeight: '100vh' }}>
          ...
          <div style={{ position: 'absolute', top: '12%', right: '12%', animation: 'float1 5s ease-in-out infinite', transform: `translate(${mouse.x * 18}px,${mouse.y * 12}px)`, transition: 'transform 0.4s ease-out' }}>
            <div style={{ width: 52, height: 52, borderRadius: 16, background: 'white', boxShadow: '0 8px 24px rgba(0,0,0,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          ...
          <div style={{ position: 'absolute', bottom: '18%', left: '8%', animation: 'float2 7s ease-in-out infinite 1s', transform: `translate(${mouse.x * -14}px,${mouse.y * 10}px)`, transition: 'transform 0.5s ease-out' }}>
            <div style={{ width: 48, height: 48, borderRadius: 14, background: 'white', boxShadow: '0 8px 20px rgba(0,0,0,0.07)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          ...
          <div style={{ position: 'absolute', top: '55%', right: '6%', animation: 'float3 6s ease-in-out infinite 2s', transform: `translate(${mouse.x * 10}px,${mouse.y * -16}px)`, transition: 'transform 0.45s ease-out' }}>
            <div style={{ width: 44, height: 44, borderRadius: 13, background: 'white', boxShadow: '0 6px 18px rgba(0,0,0,0.07)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        ...
        {/* Form side */}
        <div style={{ flex: '1 1 45%', display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '60px 56px', background: 'white', position: 'relative', animation: 'heroReveal 0.7s cubic-bezier(0.22,1,0.36,1) 0.1s both' }}>
          ...
          <div style={{ maxWidth: 360, width: '100%', margin: '0 auto' }}>
            ...
            <div style={{ marginBottom: 28, animation: 'fadeInUp 0.6s ease 0.25s both' }}>
              <RoleToggle bg="#F3F4F6" border="none" textOff="#6B7280" textOn="white" />
            </div>

            {/* Header */}
            <div key={`dh-${role}`} style={{ marginBottom: 28, animation: 'formSlideIn 0.4s ease both' }}>
              <h2 style={{ margin: '0 0 6px', fontSize: 28, fontWeight: 900, color: '#111827', letterSpacing: '-0.6px' }}>
                Bem-vindo de volta
              </h2>
